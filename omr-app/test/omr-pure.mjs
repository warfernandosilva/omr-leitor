...
var require_aruco = __commonJS({
  "node_modules/js-aruco2/src/aruco.js"(exports) {
    ...
    AR2.Dictionary = function(dicName) {
      this.codes = {};
      this.codeList = [];
      this.tau = 0;
      this._initialize(dicName);
    };
    AR2.Dictionary.prototype._initialize = function(dicName) {
      this.codes = {};
      this.codeList = [];
      this.tau = 0;
      this.nBits = 0;
      ...
      var dictionary = AR2.DICTIONARIES[dicName];
      if (!dictionary)
    ...
    AR2.Dictionary.prototype.find = function(bits) {
      var val = "", i, j;
    ...
    AR2.Dictionary.prototype._hex2bin = function(hex, nBits) {
      return hex.toString(2).padStart(nBits, "0");
    };
    AR2.Dictionary.prototype._bytes2bin = function(byteList, nBits) {
      var bits = "", byte;
    ...
    AR2.Dictionary.prototype._hammingDistance = function(str1, str2) {
    ...
    AR2.Dictionary.prototype._calculateTau = function() {
    ...
    AR2.Dictionary.prototype.generateSVG = function(id) {
    ...
    AR2.Marker = function(id, corners, hammingDistance) {
      this.id = id;
      this.corners = corners;
      this.hammingDistance = hammingDistance;
    };
    AR2.Detector = function(config) {
      config = config || {};
      ...
      this.dictionary = new AR2.Dictionary(config.dictionaryName);
      this.dictionary.tau = config.maxHammingDistance != null ? config.maxHammingDistance : this.dictionary.tau;
    };
    AR2.Detector.prototype.detectImage = function(width, height, data) {
      return this.detect({
        width,
        height,
        data
    ...
    AR2.Detector.prototype.detectStreamInit = function(width, height, callback) {
      this.streamConfig = {};
    ...
    AR2.Detector.prototype.detectStream = function(data) {
    ...
    AR2.Detector.prototype.detectMJPEGStreamInit = function(width, height, callback, decoderFn) {
      this.mjpeg = {
        decoderFn,
        chunks: [],
        SOI: [255, 216],
    ...
    AR2.Detector.prototype.detectMJPEGStream = function(chunk) {
    ...
    AR2.Detector.prototype.detect = function(image) {
    ...
    AR2.Detector.prototype.findCandidates = function(contours, minSize, epsilon, minLength) {
    ...
    AR2.Detector.prototype.clockwiseCorners = function(candidates) {
    ...
    AR2.Detector.prototype.notTooNear = function(candidates, minDist) {
    ...
    AR2.Detector.prototype.findMarkers = function(imageSrc, candidates, warpSize) {
    ...
    AR2.Detector.prototype.getMarker = function(imageSrc, candidate) {
      ...
      if (foundMin)
        return new AR2.Marker(foundMin.id, this.rotate2(candidate, 4 - rot), foundMin.distance);
      return null;
    };
    AR2.Detector.prototype.rotate = function(src) {
    ...
    AR2.Detector.prototype.rotate2 = function(src, rotation) {
...
// src/utils/card-template.ts
var CARD_WIDTH = 1448;
var CARD_HEIGHT = 2048;
var ARUCO_MARKER_SIZE = 210;
var ARUCO_IDS = { TL: 10, TR: 11, BR: 12, BL: 13 };
var halfMk = ARUCO_MARKER_SIZE / 2;
var ARUCO_MARGIN = 10;
var ARUCO_CENTERS = {
  TL: { x: halfMk + ARUCO_MARGIN, y: halfMk + ARUCO_MARGIN },
  TR: { x: CARD_WIDTH - halfMk - ARUCO_MARGIN, y: halfMk + ARUCO_MARGIN },
  BR: { x: CARD_WIDTH - halfMk - ARUCO_MARGIN, y: CARD_HEIGHT - halfMk - ARUCO_MARGIN },
  BL: { x: halfMk + ARUCO_MARGIN, y: CARD_HEIGHT - halfMk - ARUCO_MARGIN }
};
var ARUCO_HOMOGRAPHY_ORDER = ["TL", "TR", "BR", "BL"];
var QR_SIZE = 160;
var QR_CENTER = { x: CARD_WIDTH / 2, y: ARUCO_MARKER_SIZE / 2 + 10 };
var QR_PAYLOAD = "OMR|MODEL=GABARITO_01|VERSION=2";
var QR_MODEL = "GABARITO_01";
var QR_VERSION = 2;
var ALUNO_BOX = { x0: 100, y0: 170, x1: 1349, y1: 260, label: "Aluno (a):" };
var TURMA_BOX = { x0: 100, y0: 258, x1: 725, y1: 345, label: "Turma:" };
var DIVIDER_X = 723;
var DIVIDER_Y0 = 420;
var DIVIDER_Y1 = 1900;
var SUBJECT_TITLES = {
  portugues: { text: "L\xCDNGUA PORTUGUESA", centerX: 429, centerY: 480, fontSize: 36 },
  matematica: { text: "MATEM\xC1TICA", centerX: 1018, centerY: 480, fontSize: 36 }
};
var HEADER_Y = 590;
...
function buildCardTemplate() {
  ...
  return {
    ...
    qr: { size: QR_SIZE, center: QR_CENTER, payload: QR_PAYLOAD, model: QR_MODEL, version: QR_VERSION },
    identification: { aluno: ALUNO_BOX, turma: TURMA_BOX },
    divider: { x: DIVIDER_X, y0: DIVIDER_Y0, y1: DIVIDER_Y1, width: 3 },
    subjects: [
...
var ARUCO_POINTS_ARRAY = ARUCO_HOMOGRAPHY_ORDER.map((c) => ARUCO_CENTERS[c]);

// src/utils/aruco-detector.ts
var jsAruco2Module = __toESM(require_aruco(), 1);
var jsAruco2 = jsAruco2Module.default ?? jsAruco2Module;
var AR = jsAruco2.AR;
var detectorInstance = null;
function getDetector() {
  if (detectorInstance) return detectorInstance;
  detectorInstance = new AR.Detector({ dictionaryName: "ARUCO" });
  return detectorInstance;
}
var REQUIRED_IDS = [10, 11, 12, 13];
function detectArUcoMarkers(data, w, h) {
...
function validateArUcoGeometry(markers, imgW, imgH) {
  if (markers.length < 4) return false;
  const byId = {};
  for (const m of markers) byId[m.id] = m;
  const tl = byId[10], tr = byId[11], br = byId[12], bl = byId[13];
  ...
  if (topW < 0.15 * minDim || leftH < 0.15 * minDim) return false;
...
function validateQRPayload(result) {
  if (!result.valid) {
    return { ok: false, error: result.error };
  }
  if (result.model !== QR_MODEL) {
    return { ok: false, error: `Modelo incompat\xEDvel: "${result.model}" (esperado "${QR_MODEL}")` };
  }
  if (result.version !== QR_VERSION) {
    return { ok: false, error: `Vers\xE3o incompat\xEDvel: v${result.version} (esperado v${QR_VERSION})` };
  }
  return { ok: true };
}
...
var PORTUGUESE_X2 = [329.5, 419.5, 509.5, 599];
var MATHEMATICS_X2 = [919.5, 1009.5, 1098.5, 1188.5];
var BUBBLE_DIAMETER2 = 37;
var BUBBLE_RADIUS2 = BUBBLE_DIAMETER2 / 2;
var MARKER_SIZE = 34;
var ALUNO_BOX2 = { x0: 100, y0: 119, x1: 1349, y1: 206, label: "Aluno (a):", textX: 110, textY: 138 };
var TURMA_BOX2 = { x0: 100, y0: 204, x1: 725, y1: 291, label: "Turma:", textX: 110, textY: 229 };
var SUBJECT_TITLES2 = {
...
var HEADER_Y2 = 635;
var QUESTION_NUM_X2 = { portugues: 260, matematica: 850 };
var MARKER_CENTERS = [
  { x: 117.5, y: 396 },
  { x: 522, y: 396 },
  { x: 926, y: 396 },
...
function buildCardModel() {
  const makeSubject = (id, xs, titleX, questionNumX) => ({
    id,
    title: SUBJECT_TITLES2[id].text,
    centerX: titleX,
    centerY: SUBJECT_TITLES2[id].centerY,
    alternatives: xs.map((x, c) => ({ letter: VALID_ANSWERS[c], x })),
    headerY: HEADER_Y2,
    questionNumX,
    questions: QUESTION_Y2.map((y) => ({
      y,
      options: xs.map((x, c) => ({
        question: QUESTION_Y2.indexOf(y) + 1,
        alternative: VALID_ANSWERS[c],
        centerX: x,
        centerY: y,
        radius: BUBBLE_RADIUS2
      }))
    }))
  });
  return {
    ...
    identification: { aluno: ALUNO_BOX2, turma: TURMA_BOX2 },
    calibrationMarkers: MARKER_CENTERS.map((m) => ({ x: m.x, y: m.y, size: MARKER_SIZE })),
    divider: { x: DIVIDER_X2, y0: 465, y1: 1844, width: 3 },
    subjects: [
      makeSubject("portugues", PORTUGUESE_X2, SUBJECT_TITLES2.portugues.centerX, QUESTION_NUM_X2.portugues),
      makeSubject("matematica", MATHEMATICS_X2, SUBJECT_TITLES2.matematica.centerX, QUESTION_NUM_X2.matematica)
    ]
  };
}
...
function adaptiveDarkMask(gray, sw, sh) {
  const w2 = sw + 1;
  const h2 = sh + 1;
...
function labelDarkComponents(dark, sw, sh) {
...
function clusterCandidatesByX(cands, tol) {
...
function validateQuadGeometry(q, w, h) {
  const [TL, TR, BR, BL] = q;
...
function detectMarkers(data, w, h) {
  ...
  const tolX = Math.max(8, Math.round(0.02 * minDim / sx));
  ...
  const nearest = (arr, target) => arr.reduce((a, b) => Math.abs(b.y - target) < Math.abs(a.y - target) ? b : a);
  const ordered = [
    nearest(leftCands, topY),
    // TL
    nearest(rightCands, topY),
    // TR
    nearest(rightCands, botY),
    // BR
    nearest(leftCands, botY)
    // BL
  ];
...
function warpSheet(data, w, h, markers, outW, outH, templatePoints) {
...
var SAFE_INNER = 16;
function diskStats(rect, w, h, cx, cy, ox, oy, thr) {
  let sum = 0;
  let min = 255;
  let dark = 0;
  let total = 0;
...
function validateRectified(rect, outW, outH) {
  let ok = 0;
...
function validateArUcoRectified(rect, outW, outH) {
  const halfMk2 = Math.floor(ARUCO_MARKER_SIZE / 2);
  const corners = [
    ARUCO_CENTERS.TL,
    ARUCO_CENTERS.TR,
    ARUCO_CENTERS.BR,
  ...
  for (const c of corners) {
    const x0 = Math.round(c.x - halfMk2);
    const y0 = Math.round(c.y - halfMk2);
    const x1 = Math.round(c.x + halfMk2);
    const y1 = Math.round(c.y + halfMk2);
...
function readCard(data, w, h) {
  ...
  const readSubject = (xs) => QUESTION_Y2.map((y) => xs.map((x) => sampleBubbleRatio(norm, PAGE_WIDTH, PAGE_HEIGHT, x, y)));
  return {
    portuguesRatios: readSubject(PORTUGUESE_X2),
    matematicaRatios: readSubject(MATHEMATICS_X2),
    markers,
    rectified: norm
  };
}
function readCardArUco(data, w, h) {
  ...
  const readSubject = (xs) => QUESTION_Y2.map((y) => xs.map((x) => sampleBubbleRatio(norm, CARD_WIDTH, CARD_HEIGHT, x, y)));
  return {
    portuguesRatios: readSubject(PORTUGUESE_X2),
    matematicaRatios: readSubject(MATHEMATICS_X2),
    markers: photoPoints,
...
var DEBUG_FLOOR = 0.3;
var DEBUG_WEAK = 0.15;
function debugSetPx(buf, w, x, y, color) {
...
function debugDrawRect(buf, w, cx, cy, half, color) {
...
function debugDrawCircle(buf, w, cx, cy, r, color) {
...
function buildDebugImage(rect, outW, portuguesRatios, matematicaRatios) {
  ...
  const xSets = [PORTUGUESE_X2, MATHEMATICS_X2];
  for (let t = 0; t < 2; t++) {
    for (let q = 0; q < QUESTION_Y2.length; q++) {
      const ratios = ratioSets[t][q];
      const y = QUESTION_Y2[q];
      for (let c = 0; c < ratios.length; c++) {
        const r = ratios[c];
        const color = r >= DEBUG_FLOOR ? [0, 200, 0] : r >= DEBUG_WEAK ? [230, 190, 0] : [160, 160, 160];
        debugDrawCircle(out, outW, Math.round(xSets[t][c]), Math.round(y), BUBBLE_RADIUS2, color);
      }
    }
  }
  return out;
...
export {
  ALUNO_BOX2 as ALUNO_BOX,
  BUBBLE_DIAMETER2 as BUBBLE_DIAMETER,
  BUBBLE_RADIUS2 as BUBBLE_RADIUS,
  CARD_HEIGHT,
  CARD_WIDTH,
  DIVIDER_X2 as DIVIDER_X,
  HEADER_Y2 as HEADER_Y,
  MARKER_CENTERS,
  MARKER_SIZE,
  MATHEMATICS_X2 as MATHEMATICS_X,
  OUTER_MARKER_LOGICAL,
  PAGE_HEIGHT,
  PAGE_WIDTH,
  PORTUGUESE_X2 as PORTUGUESE_X,
  QUESTIONS_PER_SUBJECT,
  QUESTION_NUM_X2 as QUESTION_NUM_X,
  QUESTION_Y2 as QUESTION_Y,
  SUBJECT_TITLES2 as SUBJECT_TITLES,
  TOTAL_QUESTIONS,
  TURMA_BOX2 as TURMA_BOX,
  VALID_ANSWERS,
  buildCardModel,
  buildCardTemplate,
  buildDebugImage,
